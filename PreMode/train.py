...
def get_args():
    ...
    # data set specific
    parser.add_argument("--dataset", default=None, type=str, choices=data.__all__, help="Name of the dataset")
    parser.add_argument("--data-file-train", default=None, type=str, help="Custom training files")
    parser.add_argument(
        "--data-file-train-ddp-prefix", default=None, type=str, help="Prefix of custom training files if use DDP"
    )
    parser.add_argument("--data-file-test", default=None, type=str, help="Custom testing files")
    parser.add_argument("--data-type", default=None, type=str, help="Data type for the task")
    parser.add_argument(
        "--convert-to-onesite",
        type=bool,
        default=False,
    ...
    parser.add_argument("--loop", type=bool, default=False, help="Add self loop to nodes or not")
    parser.add_argument(
        "--max-num-neighbors", type=int, default=32, help="Maximum number of neighbors to consider in the network"
    )
    parser.add_argument(
        "--node-embedding-type",
        type=str,
    ...
    parser.add_argument("--graph-type", type=str, default="af2", help="Graph type. Choose from af2 or 1d-neighbor")
    parser.add_argument("--add-plddt", type=bool, default=False, help="Whether to add plddt or not")
    ...
    parser.add_argument("--add-sidechain", type=bool, default=False, help="Whether to add sidechain or not")
    parser.add_argument("--use-cb", type=bool, default=False, help="Whether to use CB as distance or not")
    parser.add_argument("--add-msa", type=bool, default=False, help="Whether to add msa to features or not")
    parser.add_argument("--zero-msa", type=bool, default=False, help="Whether to make msa zero")
    parser.add_argument(
        "--add-msa-contacts", type=bool, default=True, help="Whether to add msa contacts to features or not"
    )
    parser.add_argument(
        "--add-confidence", type=bool, default=False, help="Whether to add af2 predicted confidence or not"
    )
    parser.add_argument(
        "--add-ptm",
        type=bool,
    ...
    parser.add_argument(
        "--add-af2-single", type=bool, default=False, help="Whether to add alphafold single representation or not"
    )
    parser.add_argument(
        "--add-af2-pairwise",
        type=bool,
    ...
    parser.add_argument(
        "--loaded-af2-single", type=bool, default=False, help="Whether to load af2 single representation or not"
    )
    parser.add_argument(
        "--loaded-af2-pairwise",
        type=bool,
    ...
    parser.add_argument(
        "--loaded-confidence", type=bool, default=False, help="Whether to load af2 predicted confidence or not"
    )
    parser.add_argument("--loaded-msa", type=bool, default=False, help="Whether to preload msa to features or not")
    parser.add_argument("--loaded-esm", type=bool, default=False, help="Whether to preload esm to features or not")
    parser.add_argument("--alt-type", type=str, default="alt", help="alt type in data, either alt or concat")
    parser.add_argument("--computed-graph", type=bool, default=True, help="Whether to use computed graph or not")
    parser.add_argument(
        "--neighbor-type",
        type=str,
        default="KNN",
    ...
    parser.add_argument("--max-len", type=int, default=2251, help="Maximum length of input sequences")
    parser.add_argument("--radius", type=float, default=50, help="Radius of AA to be selected")
    parser.add_argument(
        "--data-augment",
        type=bool,
        default=False,
    ...
    parser.add_argument("--use-lmdb", type=bool, default=False, help="Whether to use preloaded lmdb")

    # model specific
    parser.add_argument("--load-model", type=str, default=None, help="Restart training using a model checkpoint")
    parser.add_argument(
        "--partial-load-model",
        type=bool,
        default=False,
    ...
    parser.add_argument("--use-output-head", type=bool, default=False, help="Use output head or not")
    parser.add_argument("--model-class", type=str, default=None, choices=model.__all__, help="Which model to use")
    parser.add_argument("--model", type=str, default=None, help="Which representation model to use")
    parser.add_argument("--triangular-update", type=bool, default=True, help="Whether do triangular update")
    parser.add_argument("--alt-projector", type=int, default=None, help="Alt projector size")
    parser.add_argument(
        "--neighbor-embedding",
        type=bool,
        default=False,
    ...
    parser.add_argument(
        "--x-in-channels", type=int, default=None, help="x input size, only used if different from x_channels"
    )
    parser.add_argument(
        "--x-in-embedding-type",
        type=str,
    ...
    parser.add_argument("--x-channels", type=int, default=1280, help="x embedding size")
    parser.add_argument("--x-hidden-channels", type=int, default=640, help="x hidden size")
    parser.add_argument("--vec-in-channels", type=int, default=4, help="vector embedding size")
    parser.add_argument("--vec-channels", type=int, default=64, help="vector hidden size")
    parser.add_argument(
        "--vec-hidden-channels", type=int, default=1280, help="vector hidden size, must be equal to x_channels"
    )
    parser.add_argument("--share-kv", type=bool, default=False, help="Whether to share key and value")
    parser.add_argument(
        "--ee-channels",
        type=int,
        default=None,
    ...
    parser.add_argument("--distance-influence", type=str, default="both", help="Which distance influences to use")
    parser.add_argument("--num-heads", type=int, default=16, help="number of attention heads")
    parser.add_argument("--num-layers", type=int, default=2, help="number of layers")
    parser.add_argument("--num-edge-attr", type=int, default=1, help="number of edge attributes")
    parser.add_argument("--num-nodes", type=int, default=1, help="number of nodes")
    parser.add_argument("--num-rbf", type=int, default=32, help="number of radial basis functions")
    parser.add_argument("--rbf-type", type=str, default="expnorm", help="type of radial basis functions")
    parser.add_argument("--trainable-rbf", type=bool, default=False, help="to train rbf or not")
    parser.add_argument("--num-workers", type=int, default=10, help="number of workers")
    parser.add_argument(
        "--output-model",
        type=str,
        default="EquivariantBinaryClassificationSAGPoolScalar",
        help="The type of output model",
    )
    parser.add_argument("--reduce-op", type=str, default="mean", help="The type of reduce operation")
    parser.add_argument("--output-dim", type=int, default=1, help="The dimension of output model")
    parser.add_argument(
        "--output-dim-1",
        type=int,
        default=1,
    ...
    parser.add_argument("--activation", type=str, default="silu", help="The activation function")
    parser.add_argument("--attn-activation", type=str, default="silu", help="The attention activation function")
    parser.add_argument("--drop-out", type=float, default=0.1, help="Drop out rate at each layer")
    parser.add_argument("--use-lora", type=int, default=None, help="Whether to use lora or not")

    # training specific
    parser.add_argument("--trainer-fn", type=str, default="PreMode_trainer", help="trainer function")
    parser.add_argument(
        "--freeze-representation", type=bool, default=False, help="freeze representation module or not"
    )
    parser.add_argument(
        "--freeze-representation-but-attention",
        type=bool,
        default=False,
        help="freeze representation module but without attention, or not",
    )
    parser.add_argument(
        "--freeze-representation-but-gru",
        type=bool,
        default=False,
        help="freeze representation module but without gru, or not",
    )
    parser.add_argument("--seed", type=int, default=0, help="random seed")
    parser.add_argument("--seed-with-pl", type=bool, default=False, help="Initialize with pytorch lightning seed")
    parser.add_argument("--lr", type=float, default=1e-5, help="learning rate")
    parser.add_argument(
        "--lr-factor", type=float, default=0.8, help="factor by which the learning rate will be reduced"
    )
    parser.add_argument(
        "--weight-decay",
        type=float,
    ...
    parser.add_argument(
        "--num-steps-update", type=int, default=1, help="number of steps after which to update the model"
    )
    parser.add_argument(
        "--lr-warmup-steps", type=int, default=2000, help="number of warmup steps for learning rate"
    )
    ...
    parser.add_argument("--gpu-id", type=int, default=0, help="default of gpu to use in processing the dataset")
    parser.add_argument("--num-epochs", type=int, default=10, help="number of epochs to train for")
    parser.add_argument("--loss-fn", type=str, default="binary_cross_entropy", help="loss function to use")
    parser.add_argument("--y-weight", type=float, default=1.0, help="weight of y in loss function")
    parser.add_argument("--data-split-fn", type=str, default="_by_good_batch", help="function for splitting data")
    parser.add_argument(
        "--contrastive-loss-fn",
        type=str,
        default="cosin_contrastive_loss",
        help="contrastive loss function to use",
    )
    parser.add_argument(
        "--reset-train-dataloader-each-epoch",
        type=bool,
        default=True,
        help="whether to reset train dataloader each epoch",
    )
    parser.add_argument(
        "--reset-train-dataloader-each-epoch-seed",
        type=bool,
        default=False,
        help="whether to set the seed of shuffle train dataloader each epoch",
    )
    parser.add_argument("--test-size", type=int, default=None, help="size of the test set")
    parser.add_argument("--train-size", type=float, default=0.95, help="fraction of data to use for training")
    parser.add_argument("--val-size", type=float, default=0.05, help="fraction of data to use for validation")
    parser.add_argument("--hp-tune", type=bool, default=False, help="Whether use hyperparameter tuning or not")
    parser.add_argument("--adaptive-rounds", type=int, default=6, help="active learning rounds")
    parser.add_argument("--init-fn", type=str, default=None, help="Initialization function for output model")
    parser.add_argument("--use-sub-seq", type=bool, default=False, help="Whether to use sub seq")

    # log specific
    parser.add_argument(
        "--num-save-epochs", type=int, default=1, help="number of epochs after which to save the model"
    )
    parser.add_argument(
        "--num-save-batches", type=int, default=1000, help="number of batches after which to save the model"
    )
    parser.add_argument(
        "--log-dir",
        type=str,
        default="/share/vault/Users/gz2294/RESCVE/CHPs.v1.ct/",
        help="directory for saving logs",
    )

    # script specific
    parser.add_argument("--mode", type=str, default="train_and_test", help="mode of training")
    parser.add_argument("--re-test", type=bool, default=False, help="re-test the model or not")
    parser.add_argument("--test-by", type=str, default="epoch_and_batch", help="test by batch or epoch")
    parser.add_argument("--interpret-by", type=str, default=None, help="interpret by batch or epoch")
    parser.add_argument("--interpret-step", type=int, default=None, help="interpret step")
    parser.add_argument("--interpret-epoch", type=int, default=None, help="interpret epoch")
    parser.add_argument("--out-dir", type=str, default=None, help="The output directory / file for interpret mode")
    parser.add_argument(
        "--interpret-idxes",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--save-attn", type=bool, default=False, help="Whether save attention matrix for interpret mode"
    )
    parser.add_argument(
        "--use-ig", type=bool, default=False, help="Whether to use integrated gradient for interpret mode"
    )
    parser.add_argument(
        "--use-jacob",
        type=bool,
...
def main(args, continue_train=False, four_fold=False):
    ...
    if continue_train:
        for i in range(args.num_epochs):
            if os.path.exists(
                os.path.join(args.log_dir, f"result_dict.epoch.{i}.ddp_rank.0.json")
            ) and os.path.exists(os.path.join(args.log_dir, f"model.epoch.{i+1}.pt")):
                continue
            else:
                break
...
def adaptive_main(args):
    ...
    for i in range(args.adaptive_rounds):
        ...
        if os.path.exists(os.path.join(base_log_dir, f"candidate.training.round.{i}.csv")):
            ...
            data_file_train = pd.read_csv(
                os.path.join(base_log_dir, f"data_file_train.round.{i}.csv"), index_col=0
            )
            data_file_candidate_train = pd.read_csv(
                os.path.join(base_log_dir, f"data_file_candidate.round.{i}.csv"), index_col=0
            )
        else:
            ...
            for e in range(args.num_epochs):
                if os.path.exists(
                    os.path.join(args.log_dir, f"result_dict.epoch.{e}.ddp_rank.0.json")
                ) and os.path.exists(os.path.join(args.log_dir, f"model.epoch.{e+1}.pt")):
                    continue
                else:
                    break
        ...
        x_embed_df = x_embed_df.apply(
            lambda x: x.rank(pct=True) if isinstance(x.name, str) and x.name.startswith("logits_var") else x
        )
...
def _test(args):
    ...
    my_model = my_model.to(f"cuda:{args.gpu_id}")
    my_model.eval()
    trainer = trainer_fn(hparams=args, model=my_model, stage="test", dataset=dataset, device_id=args.gpu_id)
    ...
    if "batch" in args.test_by:
        ...
        num_saved_batches = int(
            np.floor(
                np.ceil(np.ceil(train_data_size * args.train_size) / args.ngpus / args.batch_size)
                * args.num_epochs
        ...
        for step in range(args.num_save_batches, num_saved_batches * args.num_save_batches, args.num_save_batches):
...
def interpret(args, idxs=None, epoch=None, step=None, dryrun=False, four_fold=False):
    ...
    if four_fold:
        ...
        for FOLD in range(4):
            ...
            _, ys, min_loss = interpret_core(
                args, dataset, idxs=idxs, epoch=epoch, step=step, dryrun=dryrun, four_fold=True
            )
...
def interpret_core(args, dataset, idxs=None, epoch=None, step=None, dryrun=False, four_fold=False):
    ...
    if not args.hp_tune:
        ...
        if epoch is not None:
            ...
            if args.interpret_by == "epoch":
                ...
                print(f"num_saved_batches: {num_saved_batches}")
                steps = list(
                    range(args.num_save_batches, num_saved_batches * args.num_save_batches, args.num_save_batches)
                )
                for step in steps:
                    val_loss = []
                    for rank in range(args.ngpus):
                ...
                for epoch in range(args.num_epochs):
                    ...
                    for rank in range(args.ngpus):
                        if os.path.exists(f"{args.log_dir}/result_dict.epoch.{epoch}.ddp_rank.{rank}.json"):
                            try:
                                with open(
                                    f"{args.log_dir}/result_dict.epoch.{epoch}.ddp_rank.{rank}.json", "r"
                                ) as f:
                                    result_dict = json.load(f)
                                    val_loss.append(result_dict["val_loss"] + result_dict["train_loss"])
                            except:
                                print(
                                    f"error in file {args.log_dir}/result_dict.epoch.{epoch}.ddp_rank.{rank}.json"
                                )
                        else:
                            val_loss.append(np.nan)
                    val_losses.append(np.mean(val_loss))
    ...
    for idx in idxs:
        ...
        if args.use_jacob:
            x_jacob = torch.autograd.functional.jacobian(
                jacobian_fn, (x_embed.to(trainer.device, non_blocking=True))
            )
            if hasattr(batch, "score_mask"):
                x_jacobs.append(
                    x_jacob[0, np.nonzero(batch.score_mask.numpy())[1][0], :, 0, 0].detach().cpu().numpy()
                )
            else:
        ...
        if args.save_attn:
            ...
            with open(save_file_name, "wb") as f:
                pickle.dump(
                    (
                        [a.detach().cpu().numpy() for a in attn_weight_layers],
                        batch.edge_index_star.detach().cpu().numpy(),
                        dataset.data.iloc[idx],
                        (
                            dataset.mutations[idx].seq_start_orig,
                            dataset.mutations[idx].seq_end_orig,
                            dataset.mutations[idx].seq_start,
                            dataset.mutations[idx].seq_end,
                        ),
                        y.detach().cpu().numpy(),
                        x_gradients,
                    ),
    ...
    if (
        args.loss_fn == "GP_loss"
        or args.loss_fn == "weighted_loss_betabinomial"
        or args.loss_fn == "gaussian_loss"
    ):
...
def test_scalar_invariance(args):
    ...
    y = model(
        ...
        data_point["edge_index_star"],
        data_point["edge_attr"],
        data_point["edge_attr_star"],
        node_vec_attr,
        data_point["batch"],
        data_point,
        return_attn=True,
    ...
    y_rot = model(
        ...
        data_point["edge_attr"],
        data_point["edge_attr_star"],
        (node_vec_attr.permute(0, 1, 3, 2) @ rotate).permute(0, 1, 3, 2),
        data_point["batch"],
        data_point,
        return_attn=True,
    )[0]
...
def test_scalar_invariance_2(args):
    ...
    y_rot = model(
        ...
        data_point.edge_attr,
        data_point.edge_attr_star,
        (node_vec_attr.permute(0, 2, 1) @ rotate).permute(0, 2, 1),
        data_point.batch,
    )[0]
    torch.testing.assert_allclose(y, y_rot)
...
if __name__ == "__main__":
    ...
    if _args.mode == "train":
        ...
        if _args.interpret_idxes is not None:
            _args.interpret_idxes = [int(i) for i in _args.interpret_idxes.split(",")]
        interpret(
            _args, idxs=_args.interpret_idxes, step=_args.interpret_step, epoch=_args.interpret_epoch, dryrun=True
        )
        ...
        interpret(
            _args,
            idxs=_args.interpret_idxes,
            step=_args.interpret_step,
            epoch=_args.interpret_epoch,
            dryrun=False,
            four_fold=True,
        )

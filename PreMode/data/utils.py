...
class Mutation:
    ...
    def __init__(
        ...
        self.crop_fn(sub_seq_start, sub_seq_end, use_sub_seq)
    ...
    def crop_fn(self, sub_seq_start, sub_seq_end, use_sub_seq):
        ...
        # if use sub seq, then crop based on given seq start and seq end
        if use_sub_seq:
            seq_start = sub_seq_start - self.seq_start_orig + 1
            seq_end = sub_seq_end - self.seq_start_orig + 1
...
def convert_to_onesite(dataset: pd.DataFrame):
    ...
    for i in dataset_onesite.index:
        subdataset = dataset[
            (dataset["uniprotID"] == dataset_onesite.loc[i, "uniprotID"])
            & (dataset["pos.orig"] == dataset_onesite.loc[i, "pos.orig"])
        ]
...
def extract_sidechain_from_structure(structure: biotite.structure.AtomArray):
    ...
    """
    coords = get_atom_coords_residue_wise(
        [
            "CD",
            "CD1",
            "CD2",
...
def get_random_point_mutations(uniprot_id, transcript_id, seq, seq_orig_len, pos_orig, ref_aa, alt_aa, score):
    if score == -1:
...
def get_mask_predict_point_mutations(
    uniprot_id, transcript_id, seq, seq_orig_len, pos_orig, ref_aa, alt_aa, max_len=2251, af2_file=None
):
    point_mutation = MaskPredictPointMutation(
        uniprot_id, transcript_id, seq, seq_orig_len, pos_orig, ref_aa, alt_aa, max_len, af2_file
    )
...
def get_dssp_from_af2(af2_file):
    ...
    return np.concatenate(
        [
            ...
            dssp.iloc[:, 5].values[:, None] / 180 * np.pi,
        ],
        axis=1,
    )
...
def get_radius_knn_graphs_from_af2(af2_coords, center_nodes, radius, max_neighbors, loop=False, gpu_id=None):
    ...
    with torch.no_grad():
        ...
        edge_index = edge_index[
            :,
            np.isin(edge_index[0], edge_index_radius.flatten())
            & np.isin(edge_index[1], edge_index_radius.flatten()),
        ]
    return edge_index
...
def get_graphs_from_neighbor(af2_coords, max_neighbors=None, loop=False):
    ...
    edge_graph *= (
        np.tri(nodes, k=int(np.floor(max_neighbors / 2))) * np.tri(nodes, k=int(np.floor(max_neighbors / 2))).T
    )
...
def get_embedding_from_esm2(protein, check_mode=True, seq_start=None, seq_end=None):
    if isinstance(protein, str):
        ...
        if os.path.exists(file_path):
            ...
            batch_tokens = wt_orig[max(0, seq_start) : min(wt_orig.shape[0] - 1, seq_end + 1)]
        else:
            if check_mode:
                return False
            batch_tokens = np.zeros([seq_end - seq_start + 1, 5120 if ESM_MODEL_SIZE == "15B" else 1280])
    elif isinstance(protein, np.ndarray):
        batch_tokens = protein[max(0, seq_start) : min(protein.shape[0] - 1, seq_end + 1)]
    else:
...
def get_embedding_from_onehot(
    seq, seq_start=None, seq_end=None, return_idx=False, aa_dict=None, return_onehot_mat=False
):
    if aa_dict is None:
...
def get_embedding_from_esm_onehot(
    seq, seq_start=None, seq_end=None, return_idx=False, aa_dict=None, return_onehot_mat=False
):
    if aa_dict is None:
...
def get_embedding_from_onehot_nonzero(
    seq, seq_start=None, seq_end=None, return_idx=False, aa_dict=None, min_prob=0.001, return_onehot_mat=False
):
    if aa_dict is None:
        aa_dict = AA_DICT
    one_hot_mat = np.eye(len(aa_dict))
...
def get_confidence_from_af2file(af2file, pLDDT):
    ...
    if pd.isna(uniprotID) or not os.path.exists(
        f"{PAE_DATA_PATH}/{uniprotID[3:6]}/{uniprotID}-predicted_aligned_error_v4.json.gz"
    ):
        ...
        with gzip.open(
            f"{PAE_DATA_PATH}/{uniprotID[3:6]}/{uniprotID}-predicted_aligned_error_v4.json.gz", "rt"
        ) as f:
            pae = json.load(f)
    ...
    if isinstance(transcript, str):
        ...
        if matched_line:
            if check_mode:
                return True
            msa_row_attns = (
                torch.load(os.path.join(MSA_ATTN_DATA_PATH, transcript + ".row_attentions.pt")).detach().numpy()
            )
            msa_contacts = (
                torch.load(os.path.join(MSA_ATTN_DATA_PATH, transcript + ".contacts.pt")).detach().numpy()
            )
            # R file parse seq_start starting from 1, so we need to minus 1
            # only use last 6 attn layers
            msa_row_attns = msa_row_attns[
                :, (12 - NUM_LAYERS) :, :, seq_start - 1 : seq_end, seq_start - 1 : seq_end
            ]
            msa_contacts = msa_contacts[:, seq_start - 1 : seq_end, seq_start - 1 : seq_end]
            msa_pairwise = np.concatenate(
                [msa_row_attns.reshape(-1, msa_row_attns.shape[-2], msa_row_attns.shape[-1]), msa_contacts], axis=0
            ).transpose((1, 2, 0))
        else:
            if check_mode:
                return False
        ...
        raise ValueError(
            "transcript must be either a string of transcriptID" " or a tuple of msa_row_attns and msa_contacts"
        )
    return msa_pairwise
...
def load_attn_from_msa(transcript):
    if os.path.exists(os.path.join(MSA_ATTN_DATA_PATH, transcript + ".row_attentions.pt")) and os.path.exists(
        os.path.join(MSA_ATTN_DATA_PATH, transcript + ".contacts.pt")
    ):
        msa_row_attns = (
            torch.load(os.path.join(MSA_ATTN_DATA_PATH, transcript + ".row_attentions.pt")).detach().numpy()
        )
...
def _test_load():
    test_file = pd.read_csv(
        "/share/terra/Users/gz2294/ld1/Data/DMS/ClinVar.HGMD.PrimateAI.syn/training.csv", index_col=0
    )
    # idx = np.where(test_file.uniprotID == 'Q8WZ42')[0][0]
    idx = np.where(test_file["sequence.len.orig"] == 4753)[0][0]
    point_mutation = get_mutations(
        test_file["uniprotID"].iloc[idx],
    ...
    CA_coord = coords[:, 3]
    embed_data = get_embedding_from_esm2(
        point_mutation.uniprot_id, False, point_mutation.seq_start, point_mutation.seq_end
    )
    # prepare edge features
    coev_strength = get_attn_from_msa(
        point_mutation.transcript_id, point_mutation.seq, False, point_mutation.seq_start, point_mutation.seq_end
    )
    ...
    # prepare graph
    features = dict(
        pos=torch.from_numpy(CA_coord),
        x=torch.from_numpy(embed_data),

...
for gene in PTEN PTEN.bin CCR5 CXCR4 NUDT15 SNCA CYP2C9 GCK ASPA Stab $(cat scripts/gene.txt) ALL ALL.itan ALL.itan.only $(cat scripts/gene.itan.txt) $(cat scripts/gene.split.by.pos.txt) $(cat scripts/gene.pfams.txt) fluorescence
do
...
if [ "$loss_fn" == "combined_loss" ] || [ "$loss_fn" == "weighted_combined_loss" ] || [ "$loss_fn" == "GP_loss" ]; then
  for gene in $(cat scripts/gene.txt) ALL ALL.itan ALL.itan.only $(cat scripts/gene.itan.txt) $(cat scripts/gene.split.by.pos.txt) $(cat scripts/gene.pfams.txt)
  do
...
for gene in PTEN PTEN.bin CCR5 CXCR4 NUDT15 SNCA CYP2C9 GCK ASPA Stab $(cat scripts/gene.txt) ALL ALL.itan ALL.itan.only $(cat scripts/gene.itan.txt) $(cat scripts/gene.split.by.pos.txt) $(cat scripts/gene.pfams.txt)
do
  # change load model
...
need_large_window_list=$(cat scripts/gene.txt)" ALL ALL.itan ALL.itan.only "$(cat scripts/gene.itan.txt)" "$(cat scripts/gene.split.by.pos.txt)" "$(cat scripts/gene.pfams.txt)
...
for gene in $(cat scripts/gene.txt) ALL ALL.itan ALL.itan.only $(cat scripts/gene.itan.txt) $(cat scripts/gene.split.by.pos.txt) $(cat scripts/gene.pfams.txt) $added_large_window_list
do
  mkdir $1/$gene.5fold/
...
for gene in $(cat scripts/gene.txt) ALL ALL.itan ALL.itan.only
do
